-- SIMPLIFIED BOOLEAN FIX FOR BULK SALES
-- This avoids the conversion issues by handling the boolean more reliably

-- Update the bulk sales function with better boolean handling
    ...
    LOOP
        BEGIN
            ...
            DECLARE
                ...
                -- Better boolean handling - check for multiple possible values
                v_returned_empty BOOLEAN;
                v_returned_value TEXT := sale_record->>'returned_empty';
            BEGIN
                -- Handle boolean conversion more reliably
                IF v_returned_value IN ('1', 'true', 't', 'yes', 'y') THEN
                    v_returned_empty := TRUE;
                ELSIF v_returned_value IN ('0', 'false', 'f', 'no', 'n') THEN
...
SELECT 'Simplified boolean fix applied!' as status,
       'Updated bulk sales function with better boolean handling' as details;
