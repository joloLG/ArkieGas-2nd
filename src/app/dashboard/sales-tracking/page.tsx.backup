...
import { 
  ...
  calculateMonthlyTotalsFromDatabase,
...
export default function SalesTrackingPage() {
  ...
  const [, setYearlyData] = useState<YearlyData[]>([])
  ...
  const fetchSalesData = async () => {
    try {
      ...
      // Enrich transactions data with loan information
      ...
      // Calculate totals using database-specific functions
  ...
  const downloadCSV = () => {
    ...
    return [
      ...
      sale.base_price || 0,
      (sale.quantity || 0) * (sale.selling_price || 0), // Total Amount = Quantity × Selling Price
      ...
      transactionProfit?.profit_type || (sale.transaction_type === 'sale' ? 'initial_sale' : '-'),
      loanInfo?.loan_id || '-',
      loanInfo?.loan_amount || '-',
      loanInfo?.paid_amount || '-',
      loanInfo?.remaining_balance || sale.remaining_balance || '-',
      sale.excess_payment || 0,
      loanInfo?.status || 'No Loan',
      loanInfo?.last_payment_date ? new Date(loanInfo.last_payment_date).toLocaleDateString() : '-',
      sale.returned_empty ? 'Yes' : 'No',
      sale.empty_quantity_not_returned || 0,
            ...
            <tbody className="bg-white divide-y divide-gray-100">
              {sales.slice(0, 50).map((sale) => (
                <tr key={sale.id} className="hover:bg-gray-50 transition-colors duration-150">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                    ₱{calculateTransactionProfitFromDatabase(sale).toLocaleString()}
                  </td>
